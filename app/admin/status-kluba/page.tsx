...
export default function AdminClubStatus() {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success('Status kluba je uspešno sačuvan');
    } catch (error) {
      toast.error('Greška pri čuvanju statusa kluba');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <div className="mb-6 md:mb-8">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold font-playfair uppercase tracking-wider">Status Kluba</h1>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-lg space-y-6">
        ...
        >
          ...
          {saving ? 'Čuvanje...' : 'Sačuvaj Status Kluba'}
        </button>
      </div>
    </div>
  );
